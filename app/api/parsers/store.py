...
async def store():
    try:
        ...
        store_file = (await Redis.execute("GET", "fortnite:store:{0}".format(store_hash)))['details']
        if store_file and isfile(store_file) and not config.DEVELOPER_MODE:
            logging.info("Изображение магазина уже было сгенерировано раньше, берется из файла: {0}".format(store_file))
        else:
            ...
            await Redis.execute("SET", "fortnite:store:{0}".format(store_hash), store_file, "EX", 86400)
